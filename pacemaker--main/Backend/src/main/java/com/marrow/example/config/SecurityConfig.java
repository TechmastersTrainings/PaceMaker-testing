    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // Public — Course listings (shown on home page to all visitors)
                .requestMatchers(
                    org.springframework.http.HttpMethod.GET, "/api/v1/courses"
                ).permitAll()
                // Public — Video streaming (the <video> tag cannot send auth headers)
                .requestMatchers(
                    org.springframework.http.HttpMethod.GET, "/api/v1/videos/stream/**"
                ).permitAll()
                // Public — Live class listing (anyone can view schedule)
                .requestMatchers(
                    org.springframework.http.HttpMethod.GET, "/api/v1/live-classes"
                ).permitAll()
