  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 z-[130] bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-600 hover:to-teal-600 text-white font-black px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-2.5 transition-all hover:scale-105 active:scale-95 group border border-cyan-400/20"
      >
        <Sparkles className="w-5 h-5 animate-pulse text-cyan-200 group-hover:rotate-12 transition-transform" />
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              {/* Header */}
              <div className="p-6 bg-gradient-to-br from-cyan-900 to-slate-900 text-white relative overflow-hidden flex justify-between items-center">
                <div className="relative z-10">
                  <div className="flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-cyan-300" />
                    <h3 className="text-lg font-black tracking-tight">PaceMaker AI Tutor</h3>
                  </div>
                  <p className="text-cyan-200/60 text-xs mt-1">Structured explanations & textbook-grounded RAG Q&A</p>
                </div>
                <button
                  onClick={() => setIsOpen(false)}
                ...
                <div className="absolute -bottom-8 -right-8 w-24 h-24 bg-teal-500/10 rounded-full blur-2xl pointer-events-none" />
              </div>
              ...
              <div className="flex border-b border-slate-100 bg-slate-50/50 p-1.5 m-4 rounded-xl">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                    activeTab === 'explain' 
                      ? 'bg-white text-cyan-900 shadow-sm border border-slate-100' 
                    ...
                    <form onSubmit={handleExplain} className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-cyan-500 font-semibold"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-xs focus:outline-none focus:border-cyan-500 font-bold"
                          >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-3 text-xs focus:outline-none focus:border-cyan-500 font-semibold"
                          />
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-4 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-cyan-600/10 active:scale-98 transition-all disabled:opacity-50"
                      >
                        {isExplaining ? (
                          <>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {sampleTopics.map(t => (
                            <button
                              key={t}
                              onClick={() => setTopic(t)}
                              className="px-3.5 py-2 border border-slate-100 hover:border-cyan-200 bg-slate-50/50 hover:bg-cyan-50/20 text-slate-600 hover:text-cyan-900 rounded-xl text-xs font-semibold transition-all flex items-center gap-1"
                            >
                    ...
                    {explanation && (
                      <div className="space-y-4 mt-6 border-t border-slate-100 pt-6">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-black uppercase text-cyan-900 tracking-wider">AI Tutor Explanation</span>
                          <a
                            href={aiService.getExplainDownloadUrl(topic, level, context)}
                            target="_blank"
                            rel="noreferrer"
                            className="px-3 py-1.5 bg-slate-100 hover:bg-cyan-50 text-slate-700 hover:text-cyan-700 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1 transition-colors"
                          >
                  ...
                  <div className="flex flex-col h-full min-h-[400px]">
                    <div className="flex-1 space-y-4 overflow-y-auto pr-1 mb-4">
                            ...
                            <div className="flex flex-col gap-2">
                              {sampleQuestions.map(q => (
                                <button
                                  ...
                                  className="w-full text-left p-3.5 border border-slate-100 hover:border-cyan-200 bg-slate-50/50 hover:bg-cyan-50/20 text-slate-600 hover:text-cyan-900 rounded-xl text-xs font-semibold transition-all flex items-center justify-between"
                                >
                                ...
                                <div className="space-y-1">
                                  {chat.sources.map((src, sIdx) => (
                                    <div key={sIdx} className="text-[10px] text-slate-500 flex justify-between items-center font-medium">
                                      ...
                                      <span className="text-[9px] font-black text-cyan-600 uppercase tracking-widest">Score: {src.score}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>
                      ...
                      {isAsking && (
                        <div className="flex items-center gap-2 text-xs text-slate-400 font-semibold italic">
                          <Loader2 className="w-4 h-4 animate-spin text-cyan-500" /> Consulting local ChromaDB vector library...
                        </div>
                      )}
                    </div>
                    ...
                    <form onSubmit={handleAsk} className="flex gap-2 border-t border-slate-100 pt-4 mt-auto">
                      <input
                        ...
                        className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-cyan-500 font-semibold"
                      />
                      <button
                        type="submit"
                        disabled={isAsking}
                        className="p-3 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl flex items-center justify-center transition-colors disabled:opacity-50"
                      >
