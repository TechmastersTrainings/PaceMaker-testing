...
const PAGE_CONTEXT: Record<string, { title: string; greeting: string; chips: string[] }> = {
  ...
  '/dashboard/achievements': {
    title: 'Achievements',
    greeting: " Well done on your progress! Ask me what to study next or how to unlock more badges.",
...
Do NOT provide general notes. Format it strictly as a sequential list of discussion questions and answers.`;
      ...
      setMessages(prev => prev.filter(m => m.id !== 'loading').concat({
        ...
        text: 'Could not reach the AI service. Please make sure the Groq AI service is running on port 8000.'
      }));
    } finally {
      setLoading(false);
    }
  ...
  const btnColor = isInstructor
    ? 'from-cyan-500 to-teal-500 shadow-teal-500/30'
    : 'from-cyan-500 to-teal-500 shadow-teal-500/30';
        ...
        <div className="fixed bottom-6 right-6 z-[300] w-[400px] max-w-[95vw] h-[600px] max-h-[85vh] flex flex-col rounded-3xl shadow-2xl overflow-hidden border border-white/20"
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                   <p className="text-cyan-200 text-[10px] font-bold">{ctx.title} · Groq Llama 3.3</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto bg-gray-50 px-4 py-4 space-y-3 custom-scrollbar">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'ai' && (
                  <div className="w-7 h-7 bg-gradient-to-br from-cyan-500 to-teal-500 rounded-xl flex items-center justify-center shrink-0 mr-2 mt-0.5">
                ...
                <div className="flex flex-col gap-1.5 pl-9">
                  {ctx.chips.map((chip) => (
                    <button
                      ...
                      disabled={loading}
                      className="text-left text-xs font-bold text-cyan-700 bg-cyan-50 hover:bg-cyan-100 border border-cyan-100 px-3 py-2 rounded-xl transition-colors flex items-center gap-1.5 disabled:opacity-50"
                    >
          ...
          <form onSubmit={handleSubmit} className="px-4 py-3 bg-white border-t border-gray-100 flex items-center gap-2 shrink-0">
            <input
              ...
              disabled={loading}
              className="flex-1 text-sm border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-500/10 font-medium disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="w-10 h-10 bg-gradient-to-br from-cyan-600 to-teal-600 text-white rounded-xl flex items-center justify-center disabled:opacity-40 hover:opacity-90 active:scale-95 transition-all shrink-0"
            >
