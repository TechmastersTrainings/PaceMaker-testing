...
function AdminStudyMaterialPage() {
  ...
  const handleToggleStatus = async (material: StudyMaterial) => {
    const nextStatus = material.status === 'published' ? 'draft' : 'published';
    try {
      await studyMaterialService.updateMaterial(
        Number(material.id),
        material.subject,
        material.chapter,
        nextStatus
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await studyMaterialService.updateMaterial(
        Number(editingMaterial.id),
        editingMaterial.subject,
        editingMaterial.chapter,
      ...
      addToast('error', 'Failed to update material.');
    }
  };
  ...
  return (
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total Materials', value: totalMaterials, icon: FileText, color: 'text-primary-600 bg-primary-100 border-blue-100' },
          ...
          { label: 'Storage Used', value: `${storageUsed} MB`, icon: Layers, color: 'text-cyan-600 bg-cyan-50 border-cyan-100' }
        ].map((card, i) => (
