...
function AdminUsersManagementPage() {
  ...
  const fetchUsersFromDb = async () => {
    try {
      ...
      const list = res.data.map((user: any) => ({
        ...
        name: user.name || '',
        email: user.email || '',
        role: (user.role ? user.role.toLowerCase() : 'student') as UserRole,
        status: (user.status ? user.status.toLowerCase() : (user.enabled ? 'active' : 'disabled')) as UserStatus,
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || '')}&background=0d9488&color=fff&bold=true`,
        joinedDate: user.createdAt ? user.createdAt.split('T')[0] : new Date().toISOString().split('T')[0],
        lastActive: user.createdAt ? user.createdAt.split('T')[0] : new Date().toISOString().split('T')[0],
        phone: user.phone || '',
        enrolledCourses: [],
        totalSpent: 0,
        activityLog: [
          { timestamp: user.createdAt ? user.createdAt.replace('T', ' ').substring(0, 16) : new Date().toISOString().replace('T', ' ').substring(0, 16), action: "Profile loaded from database", ipAddress: "127.0.0.1" }
        ],
        coursesDetail: [],
        paymentHistory: [],
        disableReason: user.status === 'disabled' || !user.enabled ? 'Account disabled by administrator.' : ''
      }));
      setUsers(list);
    } catch (err: any) {
      console.error("Failed to load users from DB:", err);
      triggerToast('error', 'Failed to fetch users from database.');
    }
  };
  ...
  const executeChangeRole = async () => {
    ...
    try {
      await apiClient.put(`/admin/users/${roleModalUser.id}/role`, {
        role: selectedNewRole.toUpperCase()
      });
      
      setUsers(prev => prev.map(u => {
        if (u.id === roleModalUser.id) {
          return {
            ...u,
      ...
      console.error(err);
      triggerToast('error', err.message || 'Failed to change role.');
    }
  };
  ...
  const executeToggleStatus = async (user: PaceMakerUser, targetStatus: UserStatus, reason?: string) => {
    try {
      const statusParam = targetStatus === 'active' ? 'approved' : 'disabled';
      await apiClient.put(`/admin/users/${user.id}/status?status=${statusParam}`);
  ...
  const executeAddUser = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      console.error(err);
      triggerToast('error', err.message || 'Failed to add user.');
    }
  };
  ...
  const executeBulkAction = async () => {
    ...
    try {
      const targetUsers = users.filter(u => selectedUsers.has(u.email));
      
      await Promise.all(targetUsers.map(async (u) => {
        if (bulkActionType === 'disable') {
          await apiClient.put(`/admin/users/${u.id}/status?status=disabled`);
        } else if (bulkActionType === 'enable') {
          await apiClient.put(`/admin/users/${u.id}/status?status=approved`);
        } else if (bulkActionType === 'change_role') {
          await apiClient.put(`/admin/users/${u.id}/role`, { role: bulkActionTargetRole.toUpperCase() });
        } else if (bulkActionType === 'delete') {
          await apiClient.delete(`/admin/user/${u.id}`);
        }
      }));
      ...
      if (bulkActionType === 'delete') {
        ...
        triggerToast('success', `Successfully processed bulk ${bulkActionType} for ${selectedUsers.size} users.`);
      }
    } catch (err: any) {
      console.error(err);
      triggerToast('error', err.message || 'Failed to execute bulk action.');
    } finally {
      setSelectedUsers(new Set());
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {[
          ...
          { label: 'New Registrations', count: stats.newThisMonth, color: 'text-primary-600', bg: 'bg-primary-100/70 border-blue-100', icon: Calendar },
        ].map((stat, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4.5">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                          ...
                          user.role === 'instructor' ? 'bg-primary-100 text-primary-600 border border-blue-100' :
                      ...
                      <td className="px-6 py-4.5">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                          user.status === 'disabled' ? 'bg-red-50 text-red-650 border border-red-100' :
                      ...
                      <td className="px-6 py-4.5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            className="p-2 border border-gray-100 hover:border-blue-300 hover:bg-primary-100 rounded-xl text-gray-500 hover:text-blue-700 transition-all shadow-sm"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      { id: 'instructor', title: 'Instructor', color: 'border-blue-200 text-blue-650 hover:bg-primary-100 bg-primary-100/20' },
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-1.5">
                      <span className={`inline-flex items-center gap-1 px-3 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                        ...
                        selectedUserDetails.role === 'instructor' ? 'bg-primary-100 text-blue-650' :
                        ...
                        <div className="space-y-4 bg-white p-6 rounded-3xl border border-gray-200/80 shadow-sm">
                          ...
                          <div>
                            ...
                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mt-1 ${
                               selectedUserDetails.status === 'disabled' ? 'bg-red-100 text-red-800' : 
                               (selectedUserDetails.status === 'pending' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800')
                             }`}>
              ...
              <div className="px-10 py-8 bg-gray-50 border-t border-gray-100 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                     selectedUserDetails.status === 'disabled' ? 'bg-red-100 text-red-750' : 
                     (selectedUserDetails.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-750')
                   }`}>
                    {selectedUserDetails.status}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-white border border-gray-200 hover:border-blue-600 text-primary-600 text-xs font-black uppercase tracking-widest rounded-2xl transition-all flex items-center gap-2 shadow-sm"
                  >
