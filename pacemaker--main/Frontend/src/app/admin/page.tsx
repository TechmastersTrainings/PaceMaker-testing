...
function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        ...
        // Fourth: the instructor should be able to see all the videos uploaded by them as in the dashboard total
        ...
        setVideoCount(filtered.length.toString());
      } catch (err) {
  ...
  const stats = [
    { title: 'Total Subscribers', value: '0', increase: '0%', icon: Users, color: 'text-primary-500', bg: 'bg-primary-500/10' },
    ...
    { title: 'Monthly Revenue', value: '$0', increase: '0%', icon: TrendingUp, color: 'text-cyan-500', bg: 'bg-cyan-500/10' },
  ];
