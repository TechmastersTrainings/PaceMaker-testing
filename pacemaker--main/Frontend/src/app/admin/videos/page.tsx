...
import { Video, UploadCloud, Info, CheckCircle2, Film, Loader2, X, Play, Trash2, ArrowLeft, ChevronDown, Tag, Clock, Globe } from 'lucide-react';
...
function AdminVideosPage() {
  ...
  const loadVideosFromDb = async () => {
    try {
      ...
      const mapped = dbVideos.map((vid: any) => ({
        id: String(vid.id),
        title: vid.title,
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      formData.append('category', subject.toUpperCase());
      formData.append('accessLevel', accessLevel);
      formData.append('tags', tags || 'medical, lecture');
      formData.append('subject', subject);
      formData.append('assetId', assetId);
      formData.append('uploadUrl', uploadUrl);
      formData.append('instructor', instructor || 'Admin');
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await videoService.updateVideo(Number(editingVideo.id), {
        ...
        category: editForm.subject.toUpperCase(),
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary-100 text-primary-700 text-sm font-semibold mb-4 border border-primary-200">
          <Film className="w-4 h-4" /> Global Content Studio (Admin)
        </div>
        <h1 className="text-3xl font-extrabold text-gray-900">Video Management</h1>
        <p className="text-gray-500 mt-2">Manage and upload educational videos across all subjects and instructors.</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-panel p-8 rounded-3xl border border-gray-200 shadow-sm relative overflow-hidden bg-white">
            ...
            <div className="relative z-10">
              ...
              <div className="space-y-6 mb-8">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-shadow text-gray-900 font-medium outline-none" 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all flex items-center justify-between text-gray-900 font-medium outline-none"
                      >
                        <span>{subject}</span>
                      ...
                      <AnimatePresence>
                        {isSubjectDropdownOpen && (
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: 10 }}
                            className="absolute z-[110] left-0 right-0 mt-2 bg-white rounded-xl shadow-2xl border border-gray-150 overflow-hidden"
                          >
                            {['Anatomy', 'Physiology', 'Biochemistry', 'Pathology'].map((opt) => (
                              <button
                                key={opt}
                                type="button"
                                onClick={() => {
                  ...
                  {/* Fillable Instructor Input for Admin */}
                  <div>
                          ...
                          >
                            {['FREE', 'BASIC', 'MEDIUM', 'HIGH'].map((lvl) => (
                              <button
                                ...
                                className={`w-full px-4 py-3 text-left transition-colors hover:bg-primary-50 ${accessLevel === lvl ? 'bg-primary-50 text-primary-600 font-bold' : 'text-gray-700'}`}
                              >
                                {lvl}
                              </button>
                            ))}
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-shadow text-gray-900 resize-none outline-none" 
              ...
              {/* File Upload Area */}
              <div className="mt-8 relative border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center flex flex-col items-center justify-center bg-gray-50/50 transition-colors hover:border-primary-500 group overflow-hidden min-h-[200px]">
                <input 
                  type="file"
                  accept="video/*"
                  key={uploadKey}
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 flex justify-end items-center gap-6">
                ...
                <button 
                  ...
                  className="bg-primary-600 hover:bg-primary-700 text-white px-8 py-3 rounded-xl font-bold transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
                >
                  {isPublishing ? (
        ...
        <div className="space-y-6">
          <div className="glass-panel p-6 rounded-3xl border border-gray-200 shadow-sm bg-white">
             <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
               <Video className="w-5 h-5 text-primary-500" /> System Videos ({recentVideos.length})
             </h3>
                 ...
                 recentVideos.map((video) => (
                   ...
                   >
                     <div className="w-12 h-12 bg-primary-50 rounded-lg flex items-center justify-center shrink-0 relative overflow-hidden">
                       <Video className="w-5 h-5 text-primary-400 group-hover:opacity-0 transition-opacity" />
          ...
          >
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <svg className="w-5 h-5 text-primary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                Edit Video
              </h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-gray-900 font-medium" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={editForm.accessLevel} onChange={(e) => setEditForm({ ...editForm, accessLevel: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-gray-900 font-medium">
                    {['FREE', 'BASIC', 'MEDIUM', 'HIGH'].map((lvl) => (
              ...
              <div>
                ...
                <input type="text" value={editForm.instructor} onChange={(e) => setEditForm({ ...editForm, instructor: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-gray-900 font-medium" />
              </div>
              <div>
                ...
                <input type="text" value={editForm.tags} onChange={(e) => setEditForm({ ...editForm, tags: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-gray-900 font-medium" placeholder="medical, lecture" />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Description</label>
                <textarea rows={2} value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none text-gray-900 resize-none" />
              </div>
            </div>
            <div className="p-6 border-t border-gray-200 flex justify-end gap-3">
              ...
              <button onClick={handleSaveEdit} disabled={isSavingEdit}
                className="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 disabled:opacity-50 shadow-md">
