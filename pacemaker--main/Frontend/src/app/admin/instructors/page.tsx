...
import { 
  ...
  Download, ArrowLeft, ShieldCheck, User, X
} from 'lucide-react';
...
const getInitials = (inst: any) => {
  ...
  if (inst.firstName && inst.lastName) {
    return `${inst.firstName[0]}${inst.lastName[0]}`.toUpperCase();
  }
  if (inst.fullName) {
    const parts = inst.fullName.split(' ');
    if (parts.length > 1) {
...
function AdminInstructorsPage() {
  ...
  const [previewDoc, setPreviewDoc] = useState<{ label: string; filename: string } | null>(null);
    ...
    const loadInstructors = async () => {
      try {
        ...
        const list = res.data.map((user: any) => ({
          ...
          role: user.role ? user.role.toLowerCase() : 'instructor',
          status: user.status ? user.status.toLowerCase() : 'pending',
          registeredAt: user.createdAt || new Date().toISOString(),
          specialization: user.specialization,
          subSpecialization: user.subSpecialization,
          ...
          medicalCertName: user.medicalCertName,
          aadharCardName: user.aadharCardName,
          cvName: user.cvName
        }));
        setInstructors(list.sort((a: Instructor, b: Instructor) => new Date(b.registeredAt).getTime() - new Date(a.registeredAt).getTime()));
      } catch (err) {
        console.error("Failed to load instructors from DB", err);
      }
    };
  ...
  const updateStatus = async (email: string, newStatus: InstructorStatus) => {
    const inst = instructors.find(i => i.email === email);
    if (!inst) return;
    try {
      if (inst.id) {
        await apiClient.put(`/admin/users/${inst.id}/status?status=${newStatus}`);
      }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        {[
          ...
          { label: 'Total Applications', count: instructors.length, color: 'text-primary-600', bg: 'bg-primary-100', icon: Users },
        ].map((stat, i) => (
      ...
      <AnimatePresence>
        {selectedInstructor && (
          <div key="details-modal-wrapper" className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div 
              ...
              <div className="px-10 py-8 bg-gray-50/50 border-b border-gray-100 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 rounded-3xl bg-teal-600 text-white flex items-center justify-center font-black text-3xl shadow-xl shadow-teal-600/30">
                    {getInitials(selectedInstructor)}
                  </div>
                  <div>
                  ...
                  <div className="space-y-10">
                    ...
                    <section>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {[
                          { label: 'Medical Degree', filename: selectedInstructor.medicalCertName },
                          { label: 'ID Proof (Aadhar)', filename: selectedInstructor.aadharCardName },
                          { label: 'CV / Resume', filename: selectedInstructor.cvName }
                        ].map((doc) => (
                          <div key={doc.label} className="p-4 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-between group hover:border-teal-200 transition-all">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center">
                                <Download className="w-4 h-4 text-gray-400 group-hover:text-teal-600 transition-colors" />
                              </div>
                              <div>
                                <span className="text-xs font-black text-gray-700 block">{doc.label}</span>
                            ...
                            {doc.filename && (
                              <button 
                                onClick={() => setPreviewDoc({ label: doc.label, filename: doc.filename! })}
      ...
      <AnimatePresence>
        {previewDoc && (
          <div key="preview-modal-wrapper" className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <motion.div 
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-stone-200"
            >
              <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between shrink-0">
                <div>
                  <h3 className="font-black text-gray-900 text-sm">{previewDoc.label}</h3>
                  <p className="text-xs text-gray-400 font-medium">{previewDoc.filename}</p>
                </div>
                <button 
                  onClick={() => setPreviewDoc(null)}
              ...
              <div className="p-8 flex-1 overflow-y-auto bg-stone-150 flex justify-center items-center min-h-[400px] w-full">
                {previewDoc.filename && (
