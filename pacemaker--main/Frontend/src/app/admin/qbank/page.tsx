...
function AdminQBankManagement() {
    ...
    async function loadQuestions() {
      try {
        ...
        if (res && res.content && res.content.length > 0) {
          const mapped = res.content.map(mapBackendQuestion);
          setQuestions(mapped);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        const res = await qbankService.updateQuestion(Number(isEditing), formData);
        setQuestions(prev => prev.map(q => q.id === isEditing ? mapBackendQuestion(res) : q));
      } else {
        const res = await qbankService.createQuestion(formData);
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Delete this question permanently?')) {
      try {
        ...
        alert("Failed to delete question. Make sure you have permission.");
      }
    }
  };
            ...
            if (Array.isArray(data)) {
              for (const q of data) {
                try {
                  ...
                  const res = await qbankService.createQuestion(reqBody);
                  setQuestions(prev => [mapBackendQuestion(res), ...prev]);
                } catch (err) {
                  console.error("Failed to upload question via bulk:", err);
                }
              }
            }
          } catch (err) {
  ...
  const subjectOrder = [...new Set(questions.map(q => q.subject))];
  ...
  useEffect(() => {
    if (subjectOrder.length > 0) {
      setExpandedSubjects(prev => {
        if (prev.size === 0) return new Set(subjectOrder);
        return prev;
      });
    }
  }, [questions.length]);
                      ...
                      {files.map(file => (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div className={`p-2 rounded-xl ${file.type === 'json' ? 'bg-amber-50 text-amber-600' : 'bg-primary-100 text-primary-600'}`}>
           ...
           <div className="space-y-3">
              {Object.keys(groupedQuestions).length > 0 && (
                <div className="flex items-center justify-between px-1">
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{filteredQuestions.length} questions across {Object.keys(groupedQuestions).length} subjects</p>
                  <button
                    onClick={toggleAll}
                    className="text-xs font-bold text-primary-600 hover:text-primary-700 transition-colors bg-transparent border-none cursor-pointer"
                  >
                ...
                subjectQuestions.length > 0 && (
                  <div key={subject} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                    <button
                      ...
                      className="w-full flex items-center justify-between px-8 py-5 bg-gray-50/50 hover:bg-gray-50 transition-colors border-b border-gray-100 cursor-pointer"
                    >
                      <div className="flex items-center gap-4">
                        <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${expandedSubjects.has(subject) ? '' : '-rotate-90'}`} />
                        <span className="text-base font-bold text-gray-900">{subject}</span>
                        <span className="px-2.5 py-0.5 rounded-full bg-primary-50 text-primary-700 text-[10px] font-black">{subjectQuestions.length}</span>
                      </div>
                              ...
                              <div key={q.id} className="p-8 hover:bg-gray-50/50 transition-colors group">
                                <div className="flex justify-between items-start mb-4">
                                  ...
                                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEdit(q)} className="p-2 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-xl transition-all"><Edit3 className="w-4 h-4" /></button>
                                    <button onClick={() => handleDelete(q.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all"><Trash2 className="w-4 h-4" /></button>
                                  </div>
                                </div>
                                <p className="text-lg font-bold text-gray-900 leading-relaxed mb-4">{q.questionText}</p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  {Object.entries(q.options).map(([key, value]) => (
                                    <div key={key} className={`p-3.5 rounded-2xl border text-sm font-medium ${
                                      q.correctOption === key ? 'bg-green-50 border-green-200 text-green-700' : 'bg-gray-50 border-gray-100 text-gray-600'
                                    }`}>
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 md:p-10">
            ...
            >
              <div className="p-6 md:p-8 border-b border-gray-100 flex justify-between items-center bg-white">
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-2xl ${selectedDoc.type === 'json' ? 'bg-amber-50 text-amber-600' : 'bg-primary-100 text-primary-600'}`}>
