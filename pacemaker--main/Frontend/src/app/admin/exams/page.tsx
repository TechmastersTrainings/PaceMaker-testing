...
function AdminExamBuilder() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("Failed to load Q-Bank questions:", err);
      }
  ...
  const refreshBankQuestions = async () => {
    try {
      const res = await qbankService.getQuestions(undefined, undefined, undefined, 0, 100);
      if (res && res.content) {
        setBankQuestions(res.content);
  ...
  const handleSave = async () => {
    if (editingExam) {
      ...
      try {
        const exists = exams.some(e => String(e.id) === String(editingExam.id));
        if (exists) {
            ...
            <div className="p-8">
              ...
              <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => handleDuplicate(exam)} className="p-3 text-gray-400 hover:text-primary-600 hover:bg-primary-100 rounded-xl transition-all" title="Duplicate"><Copy className="w-5 h-5" /></button>
                      ...
                      <div className="max-w-4xl mx-auto">
                        <div className="flex items-center justify-between mb-8">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <button onClick={() => addQuestion('truefalse')} className="px-5 py-3 bg-primary-100 text-blue-700 rounded-xl font-bold text-sm border border-blue-100 flex items-center gap-2">
                              ...
                              <div className="flex justify-between items-start mb-6">
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                                    q.type === 'mcq' ? 'bg-primary-50 text-primary-600' : 'bg-primary-100 text-primary-600'
                                  }`}>
                                    {q.type}
                                  </span>
                                </div>
                                    ...
                                    <button
                                      ...
                                      className={`px-8 py-3 rounded-xl font-bold text-sm transition-all border ${
                                        (val === 'True' && q.tfCorrect) || (val === 'False' && !q.tfCorrect)
                                          ? 'bg-primary-600 text-white border-blue-600 shadow-lg'
                        ...
                        {bankQuestions
                          ...
                          .map(bq => (
                            <div 
                              ...
                              onClick={() => {
                                const opts = Array.isArray(bq.options)
                                  ? bq.options
                                  : [bq.options?.a || '', bq.options?.b || '', bq.options?.c || '', bq.options?.d || ''];
                                const correctVal = typeof bq.correctOption === 'number'
                                  ? bq.correctOption
                                  : (bq.correctOption ? bq.correctOption.charCodeAt(0) - 97 : 0);
                              ...
                              <div className="flex justify-between items-center mb-2">
                                <div className="flex items-center gap-1.5">
                                  {bq.subject === 'AI Generated' && (
                                    <span className="flex items-center gap-1 px-1.5 py-0.5 bg-cyan-100 text-cyan-600 rounded text-[8px] font-black uppercase tracking-widest">
