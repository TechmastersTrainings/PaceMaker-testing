...
const mapBackendSession = (bs: LiveClassResponse): LiveSession => {
  const dateTime = new Date(bs.classDateTime);
  const dateStr = bs.classDateTime.split('T')[0];
  const timeStr = bs.classDateTime.split('T')[1]?.substring(0, 5) || '12:00';
  const now = new Date();
  ...
  return {
    id: String(bs.id),
    title: bs.title,
    instructor: bs.trainerName || 'Master Faculty',
    date: dateStr,
    time: timeStr,
    duration: '60 mins',
    status: status,
    meetingLink: bs.zoomJoinUrl || `/dashboard/live/${bs.zoomMeetingId}`,
    description: bs.description || '',
    course: bs.topic || 'Medical Course',
    batch: '2026 Regular Batch',
    liveType: 'webinar',
    roomID: bs.zoomMeetingId
  };
};
...
export default function InstructorLivePage() {
  return (
    <ErrorBoundary>
      <InstructorLiveManagement />
    </ErrorBoundary>
  );
}

function InstructorLiveManagement() {
  const { user } = useAuth();
  ...
  const loadSessionsFromDb = async () => {
    try {
      ...
      const mapped = dbClasses.map(mapBackendSession);
      const ownSessions = mapped.filter(s =>
        s.instructor.toLowerCase() === instructorName.toLowerCase()
      );
      setSessions(ownSessions);
    } catch (err) {
      console.error("Failed to load live classes from backend", err);
      setSessions([]);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to save session to backend database.");
    }
    resetForm();
  };
  ...
  const resetForm = () => {
    setFormData({
      title: '', instructor: instructorName, date: '', time: '', duration: '60 mins',
  ...
  const handleEdit = (session: LiveSession) => {
    setFormData({
      title: session.title, instructor: instructorName, date: session.date, time: session.time,
    ...
    <div className="max-w-7xl mx-auto min-h-screen bg-transparent text-gray-900 p-6 md:p-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-gray-900">My Live <span className="text-teal-600">Classes</span></h1>
          <p className="text-gray-600 font-medium">Schedule and manage your medical coaching sessions. Recordings auto-delete after 30 days.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        {[
          ...
          { title: 'Upcoming', value: sessions.filter(s => s.status === 'scheduled').length, icon: Calendar, color: 'text-primary-500', bg: 'bg-primary-500/10' },
              ...
              <AnimatePresence mode="popLayout">
                {filteredSessions.length > 0 ? filteredSessions.map((session) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-6 text-center">
                      <span className={`inline-flex px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest ${session.liveType === 'webinar' ? 'bg-cyan-50 text-cyan-600' :
                  ...
                  <tr>
                    <td colSpan={4} className="py-24 text-center">
                      <div className="flex flex-col items-center justify-center opacity-40">
                        ...
                        <p className="text-sm mt-1">Schedule your first live class to get started.</p>
                      </div>
                    </td>
                  </tr>
                )}
              ...
              <div className="p-8 md:p-10">
                <div className="flex justify-between items-center mb-8 pb-6 border-b border-gray-100">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 font-medium">Configure your interactive live coaching session.</p>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-widest px-1">Your Name (Instructor)</label>
                      <input
                        type="text" value={instructorName} disabled
                        className="w-full bg-gray-100 border border-gray-200 rounded-2xl px-6 py-4 outline-none font-bold text-sm text-gray-500 cursor-not-allowed"
                      />
                    </div>
                    <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-teal-50/50 p-6 rounded-2xl border border-teal-100/50">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <option value={30}>30 Days (auto-delete)</option>
