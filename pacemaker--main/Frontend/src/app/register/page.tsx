  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === 'instructor') {
        ...
        // Pass along the redirect so user lands back at /pricing after login
        const redirectParam = redirectTo !== '/dashboard' ? `&redirect=${encodeURIComponent(redirectTo)}` : '';
        router.push(`/login?registered=true&email=${encodeURIComponent(userData.email)}&role=student${redirectParam}`);
      }
    } catch (err: any) {
      console.error(err);
