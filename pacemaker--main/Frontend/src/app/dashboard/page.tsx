    ...
    <div className="min-h-screen bg-[#fdfbf7] pt-24 pb-32 selection:bg-teal-500 selection:text-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <motion.div
          ...
          className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-4 md:p-6 mb-8"
        >
          <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search subjects, topics, videos, or study material..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-14 pr-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-medium text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all"
              />
            </div>
            <div className="flex gap-2 flex-wrap shrink-0">
              {['All', 'Subjects', 'Videos', 'QBank', 'Material'].map((cat) => (
                <button
                  key={cat}
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8 space-y-8">
              {filteredCourses.length > 0 ? filteredCourses.map((course, i) => (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm font-bold text-gray-400">No subjects match "{searchQuery}"</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-4">
                {(() => {
                  ...
                  const filteredAchievements = allAchievements.filter((a) => {
                    const matchesCategory = searchCategory === 'All' || searchCategory === 'Material';
                  ...
                  return filteredAchievements.length > 0 ? filteredAchievements.map((achievement, i) => (
                    ...
                    <div className="text-center py-6">
                      <p className="text-xs text-gray-400 font-medium">No achievements match "{searchQuery}"</p>
                    </div>
                  );
                })()}
              </div>
