...
const mapBackendExam = (be: any): Exam => {
  return {
    id: be.id,
    title: be.examTitle || '',
    subject: be.questions?.[0]?.subject || 'General',
    duration: be.timeLimitMinutes || 60,
    totalMarks: be.totalMarks || 0,
    passingPercentage: be.passingPercentage || 50,
    description: be.description || '',
    questions: be.questions?.map((q: any) => ({
      id: String(q.id),
      type: 'mcq',
      text: q.questionText,
      marks: 1,
    ...
    status: be.status || 'published',
    createdAt: be.createdAt || new Date().toISOString(),
    updatedAt: be.createdAt || new Date().toISOString(),
    attempts: 0,
  };
};
...
export const examService = {
  ...
  async getAllExams(): Promise<Exam[]> {
    try {
      ...
      const mapped = data.map(mapBackendExam);
      // Sync to localStorage so we always have an offline copy
      saveLocalExams(mapped);
      return mapped;
    } catch {
  ...
  async getExamById(id: string | number): Promise<Exam> {
    try {
      const { data } = await apiClient.get<any>(`/exams/${id}`);
      return mapBackendExam(data);
    } catch {
  ...
  async createExam(exam: Partial<Exam>): Promise<Exam> {
    ...
    const requestPayload = {
      examTitle: exam.title || 'New Exam',
      questionIds: exam.questions?.map(q => Number(q.id)).filter(id => !isNaN(id)) || [],
      timeLimitMinutes: exam.duration || 60,
      totalMarks: exam.totalMarks || exam.questions?.reduce((acc, q) => acc + (q.marks || 0), 0) || 0,
      createdBy: exam.createdBy || (typeof window !== 'undefined' ? localStorage.getItem('currentUser') : null) || 'Trainer'
    };

    try {
      const { data } = await apiClient.post<any>('/exams', requestPayload);
      const mapped = mapBackendExam(data);
      // Merge backend response into localStorage
      const all = getLocalExams().filter(e => String(e.id) !== String(mapped.id));
      saveLocalExams([...all, mapped]);
      return mapped;
    } catch {
  ...
  async updateExam(id: string | number, exam: Partial<Exam>): Promise<Exam> {
    ...
    try {
      const { data } = await apiClient.put<any>(`/exams/${id}`, requestPayload);
      const mapped = mapBackendExam(data);
