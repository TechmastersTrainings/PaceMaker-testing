...
export const studyMaterialService = {
  ...
  async uploadMaterial(subjectName: string, chapterName: string, file: File, year?: number | null): Promise<StudyMaterialResponse> {
  ...
  async updateMaterial(id: number, subjectName: string, chapterName: string, status: string, file?: File): Promise<StudyMaterialResponse> {
    const formData = new FormData();
    formData.append('subjectName', subjectName);
    formData.append('chapterName', chapterName);
    ...
    const { data } = await apiClient.put<StudyMaterialResponse>(`/study-materials/${id}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
