...
export const subscriptionService = {
  ...
  async getUserSubscription(): Promise<Subscriber> {
    // Keeps fallback support intact if needed, but routes to the controller base path
    const { data } = await apiClient.get<Subscriber>('/subscriptions');
    return data;
  },

  async createSubscription(payload: any): Promise<any> {
    // Updated to match your Spring Boot @PostMapping("/create")
    const { data } = await apiClient.post('/subscriptions/create', payload);
    return data;
  },

  async verifyPayment(payload: any): Promise<{ message: string }> {
    // Updated to match your Spring Boot @PostMapping("/verify")
